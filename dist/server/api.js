...
router.post("/import/excel", upload.single("file"), async (req, res) => {
    try {
        ...
        const isMondeFormat = ["Venda Nº", "Pagante", "CPF", "Receitas", "Tipo Pessoa"].every((col) => headers.some((h) => h === col));
        let customers;
        let trips;
        let extraMessage = "";
        if (isMondeFormat) {
...
function processMondeData(rawData) {
...
function processStandardData(rawData) {
    const customersMap = new Map();
    const trips = [];
    for (const row of rawData) {
        const cpf = String(row.cpf || "").replace(/\D/g, "");
